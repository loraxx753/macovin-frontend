      ...
      {items.map((item) => (
        <details key={item.question} className="group py-4 md:py-5">
          <summary className="flex cursor-pointer list-none items-start justify-between gap-4 text-left font-display text-xl font-semibold tracking-tight text-ink marker:content-none [&::-webkit-details-marker]:hidden">
            ...
            <span
              aria-hidden
              className="mt-1 inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-md text-lg leading-none text-ember ring-1 ring-ink/10 transition group-open:rotate-45"
            >
              +
            </span>
          </summary>
          <p className="measure mt-3 max-w-2xl text-base leading-relaxed text-ink/70 text-pretty">
            {item.answer}
          </p>
        </details>
      ))}
